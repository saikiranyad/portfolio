    ...
    <section className="section" id='contact'>
        <div className="container lg:grid lg:grid-cols-2 lg:items-stretch">
            <div className="mb-12 lg:mb-0 lg:flex lg:flex-col">
                ...
                <p className="text-zinc-400 mt-3 mb-8 max-w-[50ch] lg:max-w-[30ch]">Reach out today to discuss your project needs and start collaborating on something amazing!
                </p>
                <div className="flex items-center gap-2 mt-auto">
                    {socialLinks.map(({href,icon},key)=>(
                        <a href={href} key={key} target='_blank' className="w-12 h-12 grid place-items-center ring-inset ring-2 ring-zinc-50/5 rounded-lg transition-[background-color,color] hover:bg-zinc-50 hover:text-zinc-950 active:bg-zinc-50/80">{icon}</a>
                    ))}
                </div>
                
            </div>
            <form action="https://getform.io/f/bgdymmla" className='xl:pl-10 2xl:pl-20' method='POST'>
                    <div className='md:grid md:items-center md:grid-cols-2 md:gap-2'>
                      <div className="mb-4">
                        <label htmlFor="name" className="label">Name</label>
                        <input type="text" name='name' id='name' required placeholder='Enter your name' className="text-field" />
                    </div>
                    <div className="mb-4">
                        <label htmlFor="email" className="label">Email</label>
                        <input type="email" name='email' id='email' required placeholder='Enter your email' className="text-field" />
                    </div>  
                    </div>
                    <div className="mb-4">
                        <label htmlFor="message" className='label'>Message</label>
                        <textarea name='messgae' id='message' placeholder='Message me' required className='text-field resize-y min-h-32 max-h-80'></textarea>
                    </div>
